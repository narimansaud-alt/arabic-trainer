...
const backup=live?[]:JSON.parse(fs.readFileSync('.local/backups/book4-78-before-20260902.json','utf8'));
...
const fixture=backup.filter(w=>!(w.course_name===source.course&&['7','8'].includes(w.lesson_number)))
  .concat(expected.map((w,i)=>({...w,id:50000+i})));
const output='.local/qa/book4-dictionary-7-8-'+(live?'live':'fixture');
...
try {
 for(const width of [390,1365]) {
  ...
  await page.evaluate(async course=>{
    App.username='isolated-book4-78-audit';
  ...
  for(const l of source.lessons) {
    ...
    const result=await page.evaluate(lesson=>{
      ...
      for(const minutes of [5,10,20,25,30]) {
        ...
        daily.push({minutes,total:plan.tasks.length,unique:new Set(plan.tasks.map(t=>t.word.ar)).size,
          ...
          ...(minutes===10?{freshExact:fresh.length===freshSet.size&&fresh.every(t=>freshSet.has(t.word.ar)&&t.word.lesson===lesson),
    ...
    assert.equal(result.daily[1].freshExact,true);
    for(const plural of rows.filter(w=>w.dictionary_form==='plural'))
      assert.ok(result.daily[1].pluralRu.includes(plural.word_ru),'daily plural '+plural.word_ru);
