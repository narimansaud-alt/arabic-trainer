-- Align the server plan with the approved density of four exercises per
-- selected minute. Existing in-progress or completed plans are preserved;
-- only untouched plans for the current Moscow day are resized.
...
set target_tasks = goal_minutes * 4,
    new_target = round((goal_minutes * 4) * 0.20),
    review_target = round((goal_minutes * 4) * 0.50),
    typing_target = (goal_minutes * 4)
      - round((goal_minutes * 4) * 0.20)
      - round((goal_minutes * 4) * 0.50),
    updated_at = now()
