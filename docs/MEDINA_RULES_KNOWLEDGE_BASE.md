...
## Полный русский шарх 1-го тома в разделе «Правила» — завершён 2026-08-26

Этот этап дополняет прежнюю проверку кратких карточек и не отменяет разделение `rule_ar` / `source_text`.

- Цель этапа — поместить в публичный раздел «Правила» полный русский перевод объяснений автора, а не только краткую выжимку правил. Нельзя намеренно сокращать, пересказывать или убирать повторения, примеры, исключения, замечания и грамматические разборы автора.
- Единственный доказательный источник содержания — `C:\Users\user\Desktop\Мединский курс\Первый том\Sharkh_na_1_tom_Med_kursa.pdf`. Каждая часть уроков 1–23 повторно сверена с арабским PDF.
- Проверенные границы: L1 3–4; L2 5; L3 6–7; L4 8; L5 9; L6 10; L7 11; L8 12; L9 13; L10 14–15; L11 16; L12 17–18; L13 19–22; L14 23–25; L15 26–27; L16 28; L17 29; L18 30–31; L19 32; L20 32; L21 33–34; L22 35–36; L23 36.
- Полный публичный формат завершён миграциями `20260818130000_full_book1_lessons01_03_sharh_ru.sql`, `20260818160000_full_book1_lessons04_06_sharh_ru.sql`, отдельными миграциями уроков 7–13 от `20260819110000` до `20260819120000`, миграцией `20260826120000_full_book1_lessons14_23_sharh_ru.sql` и финальной `20260826140000_deep_audit_book1_full_sharh.sql`.
- Итоговая живая база: 23 урока, 70 публичных карточек, 147 приватных фрагментов источника. Отдельный контроль 259 примеров, 34 карточек значений и 244 арабских строк таблиц показывает 0 пропусков русского пояснения.
- Глубокий проход восстановил явную классификацию `أَمَامَ/خَلْفَ` в уроке 8, русские подписи четырёх разборов идафы в уроке 13 и полное повторённое правило чисел 3–10 в уроке 20. Две ошибки архивной расшифровки страниц 12 и 26 приведены к изображению PDF; выдуманный раздельный пример с `بَيْتُ كَ` не добавлялся.
- Существующие смысловые карточки и ID сохранять, когда это возможно. Полный материал урока распределять по карточкам соответствующих тем; не смешивать разные темы и не создавать дубликаты ради каждого абзаца.
- В публичном содержимом сохранять весь подтверждённый арабский учебный текст и рядом полный точный русский перевод. Арабские заголовки, термины, формы и примеры давать с корректной полной огласовкой и русским значением; арабские блоки размечать `lang="ar" dir="rtl"`.
- Приватный `source_text` остаётся дословной записью оригинального фрагмента и не заменяется переводом, не огласовывается заново и не смешивается с публичной формулировкой или переводом.
- Таблицы, списки, противопоставления правильных/неправильных примеров и полный `إِعْرَاب` сохранять, а не заменять описанием. Широкие таблицы должны прокручиваться внутри карточки на телефоне.
- Постоянный отчёт: `docs/MEDINA_RULES_BOOK1_FULL_SHARH_FINAL_AUDIT_20260826.md`. Регрессия закреплена комплектом `scripts/test-book1-full-sharh-*.mjs`.
- В рамках этого этапа не изменять правила других томов, словарь и книги.
- Отдельная текущая правка статического справочника глаголов: `اَلنَّهْيُ` должен быть самостоятельной темой, а не строкой внутри `اَلنَّفْيُ`. Для запрета требуется отдельная полностью огласованная таблица пяти форм второго лица с русскими значениями и пояснением признака джазма.

## Book 1 controlling workflow — source audit completed 2026-08-12; full Russian pass finalized 2026-08-26
...
## Book 1 sequential source audit — 2026-08-12; deep public-translation audit — 2026-08-26
