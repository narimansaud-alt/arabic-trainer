...
## Текущий этап: полный русский шарх 1-го тома в разделе «Правила» — 2026-08-19
...
- Единственный доказательный источник содержания — `C:\Users\user\Desktop\Мединский курс\Первый том\Sharkh_na_1_tom_Med_kursa.pdf`. Сохранённый русский перевод можно использовать как рабочую опору, но каждую часть необходимо сверять с арабским PDF; он не доказывает отсутствующий в PDF материал.
- Границы текущих уроков по PDF: урок 7 — стр. 11; урок 8 — стр. 12; урок 9 — стр. 13; урок 10 — стр. 14–15; урок 11 — стр. 16; урок 12 — стр. 17–18; урок 13 — стр. 19–22. Всегда проверять фактическое начало и окончание по изображению страницы и учитывать продолжение на следующей странице.
- Уроки 1–6 уже переведены в этот полный публичный формат миграциями `20260818130000_full_book1_lessons01_03_sharh_ru.sql` и `20260818160000_full_book1_lessons04_06_sharh_ru.sql`; их тесты являются эталоном структуры и полноты для следующих уроков.
- Текущий обязательный порядок — строго **7 → 8 → 9 → 10 → 11 → 12 → 13**. Один урок сначала читается целиком, затем сравнивается с приложением, исправляется, проверяется и только после этого начинается следующий.
...
- После каждого урока проверять: полноту относительно всех строк PDF, арабский текст и харакаты, русский перевод, порядок примеров, RTL, таблицы, размеры/цвета шрифтов и отсутствие горизонтального смещения страницы. Кратко фиксировать страницы, темы и изменения.
- После урока 13 выполнить общий автоматический и экранный аудит уроков 7–13 на desktop и mobile, применить миграцию к рабочей Supabase, прочитать данные обратно, затем выполнить обычный commit/push/deploy и проверить production.
